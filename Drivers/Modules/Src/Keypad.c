...
void Keypad_EnableEXTI(void) {
  __HAL_GPIO_EXTI_CLEAR_IT(Keypad_Handle.Col1_Pin | Keypad_Handle.Col2_Pin |
                           Keypad_Handle.Col3_Pin);
...
Key_t Keypad_Scan(void) {
  ...
  GPIO_TypeDef *rowPorts[KEYPAD_ROWS] = {
      Keypad_Handle.Row1_Port, Keypad_Handle.Row2_Port, Keypad_Handle.Row3_Port,
      Keypad_Handle.Row4_Port};
...
uint8_t Keypad_Password_GetLength(void) { return Password_Index; }

uint8_t Keypad_Password_Verify(void) {
  if (Password_Index != PASSWORD_LENGTH)
    return 0;
  for (uint8_t i = 0; i < PASSWORD_LENGTH; i++) {
    if (Password_Buffer[i] != Password_Store[i])
      return 0;
  }
  return 1;
}
