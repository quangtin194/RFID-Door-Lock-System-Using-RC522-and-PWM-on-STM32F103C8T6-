...
static const Oled_Glyph_t Oled_Font[] = {
    ...
    // --- Các chữ cái in thường còn thiếu ---
    ...
    {'y', {0x1C, 0x20, 0x40, 0x20, 0xFC}},
    {'z', {0x44, 0x64, 0x54, 0x4C, 0x44}},
    // --- Một số dấu câu/toán học phổ biến (Tùy chọn bổ sung) ---
...
static void Oled_UpdateScreen(void) {
  for (uint8_t page = 0; page < OLED_PAGES; page++) {
    ...
    Oled_WriteCommand(0x00 +
                      (OLED_COL_OFFSET & 0x0F));      /* lower column nibble */
...
void Oled_ShowLines(const char *line1, uint8_t scale1, const char *line2,
                    uint8_t scale2) {
  memset(Oled_Buffer, 0x00, sizeof(Oled_Buffer));
